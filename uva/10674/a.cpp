...
struct Point {
	...
	bool operator < (const Point &b) const { return sgn(x - b.x) == 0 ? y < b.y : x < b.x; };
...
int main() {
	...
	while (scanf("%lf%lf%lf%lf%lf%lf", x, y, r, x + 1, y + 1, r + 1) != EOF) {
		if (sgn(x[0]) == 0 && sgn(x[1]) == 0 && sgn(y[0]) == 0 && sgn(y[1]) == 0 && sgn(r[0]) == 0 && sgn(r[1]) == 0) break;
		Circle c[2];
		...
		sort(res.begin(), res.end(), [&](pair<Point, Point> a, pair<Point, Point> b) { return a.fi < b.fi; });
		for (int i = 0; i < num; ++i)
			printf("%.5f %.5f %.5f %.5f %.5f\n", res[i].fi.x, res[i].fi.y, res[i].se.x, res[i].se.y, res[i].fi.dis(res[i].se));
	}
	return 0;
}
