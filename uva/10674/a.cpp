...
int main() {
    db x[2], y[2], r[2];
    while (scanf("%lf%lf%lf%lf%lf%lf", x, y, r, x + 1, y + 1, r + 1) != EOF) {
        if (sgn(x[0]) == 0 && sgn(x[1]) == 0 && sgn(y[0]) == 0 && sgn(y[1]) == 0 && sgn(r[0]) == 0 && sgn(r[1]) == 0)
            break;
        Circle c[2];
        c[0] = Circle(x[0], y[0], r[0]);
        c[1] = Circle(x[1], y[1], r[1]);
        vector<Point> p[2];
        int num = Circle::tangentLineCC(c[0], c[1], p[0], p[1]);
        printf("%d\n", num);
        if (num <= 0)
            continue;
        vector<pair<Point, Point>> res;
        for (int i = 0; i < num; ++i) {
            res.push_back(make_pair(p[0][i], p[1][i]));
        }
        sort(res.begin(), res.end(), [&](pair<Point, Point> a, pair<Point, Point> b) {
            return a.fi < b.fi;
        });
        for (int i = 0; i < num; ++i)
            printf("%.5f %.5f %.5f %.5f %.5f\n", res[i].fi.x, res[i].fi.y, res[i].se.x, res[i].se.y,
                    res[i].fi.dis(res[i].se));
    }
    return 0;
}
