...
struct Circle {
    ...
    static int tangentLineCC(Circle A, Circle B, vector<Point> &a, vector<Point> &b) {
        ...
        if (A.r < B.r)
            swap(A, B), swap(a, b);
        int d2 = (A.p.x - B.p.x) * (A.p.x - B.p.x) + (A.p.y - B.p.y) * (A.p.y - B.p.y);
        int rdiff = A.r - B.r;
        int rsum = A.r + B.r;
        if (d2 < rdiff * rdiff)
            return 0;  //内含
        ...
        //有外公切线
        db ang = acos((A.r - B.r) / sqrt(d2 * 1.0));
        a.push_back(A.getPoint(base + ang));
        b.push_back(B.getPoint(base + ang));
        ++cnt;
        a.push_back(A.getPoint(base - ang));
        b.push_back(B.getPoint(base - ang));
        ++cnt;
        if (d2 == rsum * rsum) {  //一条内公切线
            ...
            db ang = acos((A.r + B.r) / sqrt(d2 * 1.0));
            a.push_back(A.getPoint(base + ang));
            b.push_back(B.getPoint(PI + base + ang));
            ++cnt;
            a.push_back(A.getPoint(base - ang));
            b.push_back(B.getPoint(PI + base - ang));
            ++cnt;
...
struct Polygon {
        ...
        for (int u = 0, v = 1; u < sze(); ++u) {
            ...
            while (1) {
                ...
                if (diff <= 0) {
                    ...
                    if (diff == 0)
                        ans = max(ans, p[u].dis2(p[(v + 1) % sze()]));
                    break;
                }
                v = (v + 1) % sze();
            }
    ...
    db disConvexToConvex(Polygon &q) {
        int _p = 0, _q = 0;
        for (int i = 1; i < sze(); ++i)
            if (sgn(p[i].y - p[_p].y) < 0)
                _p = i;
        for (int i = 1; i < q.sze(); ++i)
            if (sgn(q[i].y - q[_q].y) > 0)
                _q = i;
        db ans = p[_p].dis(q[_q]);
        for (int i = 0; i < sze(); ++i) {
            while (1) {
                int diff = sgn(((p[(_p + 1) % sze()] - p[_p]) ^ (q[(_q + 1) % q.sze()] - p[_p])) -
                               ((p[(_p + 1) % sze()] - p[_p]) ^ (q[_q] - p[_p])));
                if (diff > 0) {
                    _q = (_q + 1) % q.sze();
                    continue;
                }
    ...
    Polygon convexMinRectangleCover() {
        ...
        for (int i = 0; i < sze(); ++i) {
            int _i = (i + 1) % sze();
            while (sgn(((p[_i] - p[i]) ^ (p[u] - p[i])) - ((p[_i] - p[i]) ^ (p[(u + 1) % sze()] - p[i]))) < 0)
                u = (u + 1) % sze();
            while (sgn((p[_i] - p[i]) * (p[r] - p[i]) - (p[_i] - p[i]) * (p[(r + 1) % sze()] - p[i])) <= 0)
                r = (r + 1) % sze();
            if (i == 0)
                l = r;
            while (sgn((p[_i] - p[i]) * (p[l] - p[i]) - (p[_i] - p[i]) * (p[(l + 1) % sze()] - p[i])) > 0)
                l = (l + 1) % sze();
...
struct halfplanes {
    ...
    halfplane &operator[](int x) {
        return hps[(x + sze()) % sze()];
    }
    void add(halfplane hp) {
        hps.push_back(hp);
    }
    void unique() {
        int m = 1;
    ...
    Polygon getConvex() {
        deq[st - 1] = deq[ed];
        Polygon po;
        for (int i = st; i <= ed; ++i) po.add(deq[i].crossPoint(deq[i - 1]));
        return po;
    }
};
...
int main() {
    ...
    while (scanf("%s", op + 1) != EOF) {
        if (strcmp(op + 1, "CircumscribedCircle") == 0) {
            Point a, b, c;
            a.scan();
            b.scan();
            c.scan();
        ...
        } else if (strcmp(op + 1, "InscribedCircle") == 0) {
            Point a, b, c;
            a.scan();
            b.scan();
            c.scan();
            Circle cir(a, b, c, 1);
            printf("(%.6f,%.6f,%.6f)\n", cir.p.x, cir.p.y, cir.r);
        } else if (strcmp(op + 1, "TangentLineThroughPoint") == 0) {
            Circle cir;
            cir.scan();
            Point p;
            p.scan();
            Line u, v;
            int num = cir.tangentLine(p, u, v);
            if (num == 0)
                puts("[]");
            else if (num == 1)
                printf("[%.6f]\n", u.getAngle() / PI * 180);
            else {
                db A = u.getAngle() / PI * 180, B = v.getAngle() / PI * 180;
                if (sgn(A - B) > 0)
                    swap(A, B);
                printf("[%.6f,%.6f]\n", A, B);
            }
        } else if (strcmp(op + 1, "CircleThroughAPointAndTangentToALineWithRadius") == 0) {
            Point p;
            p.scan();
            Line u;
            u.scan();
            ...
            scanf("%lf", &r);
            Circle c1, c2;
            int num = Circle::getCircle(u, p, r, c1, c2);
            if (num == 0)
                puts("[]");
            else if (num == 1)
                printf("[(%.6f,%.6f)]\n", c1.p.x, c1.p.y);
        ...
        } else if (strcmp(op + 1, "CircleTangentToTwoLinesWithRadius") == 0) {
            Line u, v;
            u.scan();
            v.scan();
            db r;
            scanf("%lf", &r);
            Circle c1, c2, c3, c4;
            Circle::getCircle(u, v, r, c1, c2, c3, c4);
            Point res[4] = {c1.p, c2.p, c3.p, c4.p};
            sort(res, res + 4);
            putchar('[');
            for (int i = 0; i < 4; ++i) printf("(%.6f,%.6f)%c", res[i].x, res[i].y, ",]"[i == 3]);
            puts("");
        } else {
            Circle cx, cy, c1, c2;
            cx.scan();
            ...
            db r;
            scanf("%lf", &r);
            int num = Circle::getCircle(cx, cy, r, c1, c2);
            if (num == 0)
                puts("[]");
            else if (num == 1)
                printf("[(%.6f,%.6f)]\n", c1.p.x, c1.p.y);
