...
struct Polygon {
    ...
    //判断点和三角形的关系
    bool inTriangle(Point q) {
        for (int i = 0; i < 3; ++i)
            for (int j = i + 1; j < 3; ++j)
                if (Line(p[i], p[j]).pointOnSeg(q))
                    return true;
        db area = getArea();
        if (area == 0)
            return false;
        db s = (p[0].y * p[2].x - p[0].x * p[2].y + (p[2].y - p[0].y) * q.x + (p[0].x - p[2].x) * q.y) / area;
        if (s <= 0)
            return false;
        db t = (p[0].x * p[1].y - p[0].y * p[1].x + (p[0].y - p[1].y) * q.x + (p[1].x - p[0].x) * q.y) / area;
        return t > 0 && 1 - s - t > 0;
    }
    //判断点和凸包的关系 O(log n)
    // 1 内部
