    ...
    struct node {
        ...
        void norm() {
            if (pre == len) {
                ...
                cnt += suf / 2;
                if (pre % 2)
                    hashVal = Hash::one + hashVal;
                if (suf % 2)
                    hashVal = hashVal + Hash::one;
            }
        }
        ...
        node operator+(const node &other) const {
            ...
            if (pre != len && other.pre != other.len) {
                ...
                res.hashVal = hashVal;
            else if (other.pre != other.len)
                res.hashVal = other.hashVal;
            return res;
        }
        void print() {
            dbg(pre, suf, cnt, len, hashVal.val, hashVal.len);
        }
    } t[N << 2];
