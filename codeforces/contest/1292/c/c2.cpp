...
int n, fa[N], anc[N][N], sze[N], deep[N], dis[N][N], in[N], out[N];
ll f[N][N];
vector<vector<int>> G;
void pre(int u) {
    sze[u] = 1;
...
void run() {
    ...
    for (int i = 1; i <= n; ++i) {
        if (vec[i].empty())
            break;
        for (auto &it : vec[i]) {
            if (in[it.fi] > in[it.se])
                swap(it.fi, it.se);
            if (in[it.se] >= in[it.fi] && in[it.se] <= out[it.fi]) {
                chmax(f[it.fi][it.se], 1ll * (n - sze[anc[it.fi][it.se]]) * sze[it.se] +
                                               max(f[it.fi][fa[it.se]], f[it.se][anc[it.fi][it.se]]));
            } else {
