...
void getdeep(int u, int fa) {
    chadd(res, 1ll * seg[0].query(1, 1, m, l[u], r[u]) * inv[r[u] - l[u] + 1] % mod);
    chadd(res, 1ll * seg[1].query(1, 1, m, l[u], r[u]) * inv[r[u] - l[u] + 1] % mod * deep[u] % mod);
    for (auto &v : G[u])
        if (v != fa && !vis[v]) {
            deep[v] = deep[u] + 1;
            getdeep(v, u);
...
void run() {
    ...
    G.clear();
    G.resize(n + 1);
    for (int i = 1; i <= n; ++i) l[i] = rd(), r[i] = rd(), P = P * (r[i] - l[i] + 1) % mod;
