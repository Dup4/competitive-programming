...
int gao() {
	tvec.clear(); 
	bool F = 0; 
	for (auto &u : vec) if (!a[u].empty()) {
		pII tmp = pII(u, a[u].back());
		if (tmp.fi > tmp.se) swap(tmp.fi, tmp.se);
		if (se.empty()) se.insert(tmp);
		else {
			auto pos = se.lower_bound(tmp);
			if (*pos == tmp) {
				se.erase(tmp);
				tvec.push_back(tmp.fi);
				tvec.push_back(tmp.se);
			    F = 1; 	
			} else se.insert(tmp);
		}
	}
	vec = tvec;
	for (auto &u : vec) a[u].pop_back();
	return F;
}
bool done() {
	for (int i = 1; i <= n; ++i)
