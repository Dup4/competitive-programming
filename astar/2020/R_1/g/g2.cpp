...
namespace aby{
    ...
    void init(int node){
        ...
        T=S+1;
        rep(i,1,T)head[i]=0;
        rep(i,1,tot)np[i]=p[i]=flow[i]=0;tot=1;
    }
    void add(int a,int b,int w){
        ++tot;p[tot]=b;np[tot]=head[a];head[a]=tot;flow[tot]=w;
        ++tot;p[tot]=a;np[tot]=head[b];head[b]=tot;flow[tot]=0;
    }
    int q[N],dis[N];
    int bfs(){
        rep(i,1,T)dis[i]=-1;dis[S]=0;
        q[q[0]=1]=S;
        rep(i,1,q[0]){
            int x=q[i];
...
inline int dis(int x, int y, int nx, int ny) { return abs(x - nx) + abs(y - ny); }
