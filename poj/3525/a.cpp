...
bool ok(db d) {
    halfplanes hps1;
    for (int i = 0; i < hps.sze(); ++i) hps1.add(halfplane(hps[i].move(d)));
    return hps1.halfPlaneIntersect();
}
...
int main() {
    ...
    while (scanf("%d", &n), n) {
        ...
        Polygon po;
        po.scan(n);
        for (int i = 0; i < n; ++i) hps.add(halfplane(po[i], po[i + 1]));
        db l = 0, r = 10000, res = 0;
        while (sgn(r - l) != 0) {
            db mid = (l + r) / 2;
            if (ok(mid)) {
                res = mid;
