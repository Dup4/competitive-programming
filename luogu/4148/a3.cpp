...
struct KDTree {
	...
	bool cmp(const Point &a, const Point &b, int div) { return a.x[div] == b.x[div] || cmpx(div)(a, b); }
	struct Node {
		Point e;
		db Min[DIM], Max[DIM]; 
		Node *ls, *rs; 
    ...
    struct MStack {
		Node pool[N], *tail;
		Node* bc[N]; int top; 
		void init() { top = 0; tail = pool; } 
		Node* get() {
			if (!top) return tail++;
			return bc[top--];
		}
		void save(Node *t) { bc[++top] = t; }
	}mStack;	
	void init() { mStack.init(); root = nullptr; } 
	Node* newnode() { Node* tmp = mStack.get(); tmp->init(); return tmp; }
	void pushup(Node* a) {
		a->sze = 1;
		a->sum = a->e.v;
		for (int i = 0; i < K; ++i) {
	...
	Node* build(int l, int r, vector <Point> &vec) { 
		...
		for (int i = l; i <= r; ++i) 
			for (int j = 0; j < K; ++j) 
				vax[j] += (vec[i].x[j] - avx[j]) + (vec[i].x[j] - avx[j]);
		Node* p = newnode();
		p->div = 0;
		for (int i = 1; i < K; ++i) {
			if (vax[i] > vax[p->div])
	...
	int query(Node* t, db Min[DIM], db Max[DIM]) { 
		if (!t || t->Out(Min, Max)) return 0; 
		if (t->In(Min, Max)) return t->sum;
		int res = 0;
		if (t->e.In(Min, Max)) res += t->e.v;
