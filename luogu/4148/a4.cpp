...
struct KDTree {
	...
	bool cmp(const int &a, const int &b, int div) { return s[a].x[div] <= s[b].x[div]; }
	struct Node {
		int e;
		Node *ls, *rs; 
		db Min[DIM], Max[DIM]; 
	...
	Node* build(int l, int r, vector <int> &vec) { 
		...
		for (int i = l; i <= r; ++i) 
			for (int j = 0; j < K; ++j) 
				vax[j] += (s[vec[i]].x[j] - avx[j]) + (s[vec[i]].x[j] - avx[j]);
		Node* p = newnode();
		p->div = 0;
		for (int i = 1; i < K; ++i) {
			if (vax[i] > vax[p->div])
	...
	int query(Node* t, db Min[DIM], db Max[DIM]) { 
		...
		int res = 0;
		if (s[t->e].In(Min, Max)) res += s[t->e].v;
