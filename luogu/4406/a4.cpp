...
bool inter(Line l1, Line l2) {
    return max(l1.s.x, l1.e.x) > min(l2.s.x, l2.e.x) - eps && max(l2.s.x, l2.e.x) > min(l1.s.x, l1.e.x) - eps &&
           max(l1.s.y, l1.e.y) > min(l2.s.y, l2.e.y) - eps && max(l2.s.y, l2.e.y) > min(l1.s.y, l1.e.y) - eps &&
...
bool check_inside(Point t, vector<Point> &p) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (sgn(side.s.y - side.e.y) == 0)
            continue;
        if (onseg(side.s, ray))
            res ^= (sgn(side.s.y - side.e.y) > 0);
        else if (onseg(side.e, ray))
            res ^= (sgn(side.e.y - side.s.y) > 0);
        else
            res ^= inter(ray, side);
    }
    return res;
