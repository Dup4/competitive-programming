...
int dcmp(D x) { return x < -eps ? -1 : x > eps; }
bool zero(P v) { return !dcmp(v.X) && !dcmp(v.Y); }

D sqr(D x) { return x*x; }
D dis(P a, P b) { return abs(a-b); }

D cross(P a, P b) { return a.X*b.Y - a.Y*b.X; }
D cross(P a, P b, P c) { return cross(b-a,c-a); }
D dot(P a, P b) { return a.X*b.X + a.Y*b.Y; }
D dot(P a, P b, P c) { return dot(b-a,c-a); }
...
P dir(line ln) { return ln.t - ln.s; }
P normal(P v) { return P(-v.Y,v.X); }
P unit(P v) { return v/abs(v); }

D pos(P p, line ln){
	return dot(p-ln.s,dir(ln))/norm(dir(ln));
}

void unionarea(vector<P> p[], int n, D tot[]){
	forto(i,0,n) tot[i]=0;
	for(int i=0; i<n; ++i)
	for(size_t ii=0; ii<p[i].size(); ++ii){
		...
		for(size_t j=1, k=c[0].second; j<c.size(); ++j){
