...
inline void calc(int* h, int* o, int d, int k) {
    ...
    for (int i = 0; i <= d + d; ++i) B[i] = qp(i - d + k, MOD - 2);
    if (K <= (1 << 16)) {
        fft(A, 0);
        fft(B, 0);
    ...
    } else  //这里可能是错的，注意
    {
        ...
        fft(B, 0);
        fft(B + K, 0);
        for (int i = 0; i < K; ++i) C[i + K] = (A[i] * (ll)B[i + K] + (ll)B[i] * A[i + K]) % MOD;
        for (int i = 0; i < K; ++i) C[i] = A[i] * (ll)B[i] % MOD;
        fft(C, 1);
        fft(C + K, 1);
    }
    for (int i = 0; i <= d; ++i) {
        // i+k-d...i+k
        o[i] = C[i + d] * ff[i + k - off] % MOD * qp(ff[i + k - d - off - 1], MOD - 2) % MOD;
        (o[i] < 0) ? (o[i] += MOD) : 0;
    }
}
int V;
...
inline void work(int x, vector<int>& v) {
    ...
    if (x & 1) {
        work(x - 1, v);
        for (int i = 0; i < v.size(); ++i) v[i] = (ll)v[i] * (i * V + x) % MOD;
        ll p = 1;
        for (int i = 1; i <= x; ++i) p = p * (V * x + i) % MOD;
        v.pb(p);
        return;
    }
    int d = x >> 1;
    ...
    v.resize(x + 1);
    for (int i = 0; i <= x; ++i) v[i] = aa[i] * (ll)bb[i] % MOD;
}
