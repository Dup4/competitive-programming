...
struct SEG {
	...
	void update(int &now, int pre, int l, int r, int pos, int fa, int sze) { 
		...
		int mid = (l + r) >> 1;
		if (pos <= mid) update(t[now].ls, t[pre].ls, l, mid, pos, fa, sze);
		else update(t[now].rs, t[pre].rs, mid + 1, r, pos, fa, sze); 
	}
	//fi 父亲 se 子树大小
