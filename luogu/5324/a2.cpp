...
void Modify(int now, int l, int r, int L, int R, int w) {
    ...
    pushdown(now);
    if (L <= mid)
        Modify(lson, l, mid, L, R, w);
    if (R > mid)
        Modify(rson, mid + 1, r, L, R, w);
    pushup(now);
}
...
void Modify(int x, int w) {
    ...
    num[py + x] += w;
    if (x <= n)
        Modify(1, 1, N, py + x - k + 1, py + x - k + 1, w);
}
int main() {
    n = read();
    m = read();
    ...
    while (m--) {
        int p = read(), x = read();
        if (p > 0)
            Modify(a[p] + L, -1), a[p] = x - L, Modify(a[p] + L, 1);
        else if (x > 0) {
            --py;
            ++L;
            int pos = py + n + 1;
            if (num[pos] > 0)
                Modify(1, 1, N, pos - num[pos] + 1, pos, -1);
        } else {
            int pos = py + n + 1;
            ++py;
            --L;
