...
struct BIT {
    ...
    void update(int x, int y, int val) {
        for (int i = x; i <= n; i += i & -i)
            for (int j = y; j <= n; j += j & -j) a[i][j] += val;
    }
    int query(int x, int y) {
        int res = 0;
        for (int i = x; i; i -= i & -i)
            for (int j = y; j; j -= j & -j) res += a[i][j];
        return res;
    }
    int query(int x[], int y[]) {
        int res = 0;
...
void solve(int l, int r, int L, int R) {
    if (l > r)
        return;
    if (L == R) {
        for (int i = l; i <= r; ++i) ans[que[i].id] = arr[L].v;
        return;
    }
    int MID = (L + R) >> 1;
...
int main() {
    while (scanf("%d%d", &n, &q) != EOF) {
        ...
        bit.init();
        for (int i = 1; i <= n; ++i)
            for (int j = 1; j <= n; ++j) arr[++m].scan(i, j);
        sort(arr + 1, arr + 1 + m);
