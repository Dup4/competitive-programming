...
struct MCT {
	...
	struct E { int to, nx; ll w; }e[N * 2]; int h[N];
	void init() { memset(h, 0, sizeof h); }
	void addedge(int u, int v, ll w) { e[++*h] = E{v, h[u], w}; h[u] = *h; }
	...
	void build(int l, int r) {
		...
		for (int i = l; i <= r; ++i) {
			...
			if (dinic.dep[node[i]] != -1) tmpS[++cs] = node[i];
			else tmpT[++ct] = node[i];
		}
	...
	int dep[N], fa[N][M]; ll min_dis[N][M];
	void bfs(int s) {
		queue <int> q; 
		dep[s] = 0; fa[s][0] = 1; min_dis[s][0] = 0;
		q.push(s);
		while (!q.empty())   {
			int u = q.front(); q.pop();  
			for (int i = 1; i < M; ++i) { 
