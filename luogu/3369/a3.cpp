...
#define rs(o) t[o].rs
    ...
    void del(int o, int Rank) {
        ...
        --t[o].valid;
        if (Rank <= t[ls(o)].valid + !t[o].del)
            del(ls(o), Rank);
        else
            del(rs(o), Rank - t[ls(o)].valid - !t[o].del);
    }
} scapegoatTree;
...
int main() {
    ...
    while (n--) {
        ...
        scanf("%d%d", &opt, &x);
        if (opt == 1)
            scapegoatTree.insert(scapegoatTree.rt, x);
        if (opt == 2)
            scapegoatTree.del(scapegoatTree.rt, scapegoatTree.getRank(scapegoatTree.rt, x));
        if (opt == 3)
