...
struct SEG {
    ...
    void update(int id, int l, int r, int ql, int qr, db k, db b, int pos) {
        ...
        int mid = (l + r) >> 1;
        if (ql <= mid)
            update(id << 1, l, mid, ql, qr, k, b, pos);
        if (qr > mid)
            update(id << 1 | 1, mid + 1, r, ql, qr, k, b, pos);
    }
    ...
    int query(int id, int l, int r, int x) {
        ...
        int mid = (l + r) >> 1;
        if (x <= mid)
            chmax(pos, query(id << 1, l, mid, x), x);
        else
            chmax(pos, query(id << 1 | 1, mid + 1, r, x), x);
        return pos;
    }
} seg;
