...
#define son4(u) ((u << 2) | 1)
    ...
    struct node {
        ...
        int calc() {
            return (int)(y[1] - y[0] + 1) * (int)(x[1] - x[0] + 1);
        }
        void add(int val) {
            lazy += val;
            sum += calc() * val;
    ...
    void pushup(int now) {
        a[now].sum = 0;
        for (int i = 0; i < 4; ++i) a[now].sum += a[4 * now - 2 + i].sum;
    }
    void pushdown(int now) {
        if (!a[now].lazy)
            return;
        for (int i = 0; i < 4; ++i) a[4 * now - 2 + i].add(a[now].lazy);
        a[now].lazy = 0;
    }
    ...
    void update(int now, short x0, short y0, short x1, short y1, int val) {
        if (x0 > a[now].x[1] || x1 < a[now].x[0])
            return;
        if (y0 > a[now].y[1] || y1 < a[now].y[0])
            return;
        ...
        pushdown(now);
        for (int i = 0; i < 4; ++i) update(4 * now - 2 + i, x0, y0, x1, y1, val);
        pushup(now);
    }
    int query(int now, short x0, short y0, short x1, short y1) {
        ...
        pushdown(now);
        int res = 0;
        for (int i = 0; i < 4; ++i) res += query(4 * now - 2 + i, x0, y0, x1, y1);
        pushup(now);
        return res;
    }
} seg;
...
int main() {
    ...
    while (scanf("%s", op) != EOF) {
        if (op[0] == 'X') {
            ...
            for (int i = 0; i < 2; ++i) scanf("%hd%hd", x + i, y + i);
            if (x[0] > x[1])
                swap(x[0], x[1]);
            if (y[0] > y[1])
                swap(y[0], y[1]);
