...
inline void Prework(int id) {
    ...
    for (int i = 1; i < 24; i++)
        wn[id][1][i] = pow(G[id], (P[id] - 1) / (1 << i), P[id]),
        wn[id][0][i] = pow(invG[id], (P[id] - 1) / (1 << i), P[id]);
}
...
inline void NTT(int *A, int n, int typ, int id) {
    ...
    if (typ == 0)
        for (int i = 0, inv = pow(n, P[id] - 2, P[id]); i < n; i++) A[i] = 1ll * A[i] * inv % P[id];
}
