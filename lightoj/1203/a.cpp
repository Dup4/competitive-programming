...
struct Line {
	...
	int lineCrossLine(Line b) {
		if ((*this).parallel(b)) return b.relation(s) == 3;
		return 2;
	}
	//求两直线交点
	//要保证两直线不平行或重合
...
struct Circle {
	...
	db areaIntersectTriangle(Point a, Point b) {
		...
		if (pointCrossLine(l, q[1], q[2]) == 2) {
			...
			if (sgn((a - q[2]) * (b - q[2])) < 0) q[len++] = q[1];
		}
		q[len++] = b;
...
struct Polygon {
	...
	void scan(int n) { p.resize(n); for (int i = 0; i < n; ++i) p[i].scan(); }
	vector<Line> getline() { 
		vector <Line> l(sze());
		for (int i = 0; i < sze(); ++i) {
			int j = (i + 1) % sze();
	...
	for (int kase = 1; kase <= _T; ++kase) {
		...
		if (n <= 2) puts("0");
			...
			for (int i = 0; i < po.sze(); ++i) { 
				int j = (i + 1) % po.sze();
				int k = (j + 1) % po.sze();
				angle = min(angle, po[j].rad(po[i], po[k]) * 180 / PI);
			}
			printf("%.10f\n", angle);
		}
	}
