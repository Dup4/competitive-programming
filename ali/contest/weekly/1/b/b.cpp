...
void dfs(vector<pair<int, vector<int>>> mem[], vector<int>& ref, int digits, int cur, int idx) {
    if (idx == digits) {
        ...
        for (int i = 1; i <= 9; i++) dfs(mem, ref, digits, cur * 10 + i, idx + 1);
    } else {
        ...
        if (diff == 0)
            ...
            for (int sign : {-1, 1}) {
                ...
                if (val < 0 || val > 9)
                    ...
                    dfs(mem, ref, digits, cur * 10 + val, idx + 1);
            }
        }
    }
}
...
void gao() {
    ...
    for (int digits = 2; digits <= maxdigits; digits++)
        for (int i = 1; i <= digits - 1; i++)
            for (auto& ref : luckies[i]) dfs(luckies, ref.second, digits, 0, 0);

    for (int digits = 1; digits <= maxdigits; digits++)
        for (auto& v : luckies[digits]) lucky_nums.push_back(v.first);
