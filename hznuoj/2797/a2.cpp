...
int dfs(int pos, int S, int limit, int flag) {
	...
	if (pos < 0) return 0;
	if (f[pos][S][limit][flag] != -1) return f[pos][S][limit][flag]; 
	int &res = f[pos][S][limit][flag];
	if (pos == 0) {
		if (S == 0 && flag == 0 && limit == 0) res = 1;
		else res = 0;
		return res;
	}
	res = 0;
	if (limit) {
		int _S = (1ll * S + 1ll * a[pos] * bit[n - pos] % Mod + - a[pos] + Mod) % Mod;
	//	dbg(_S);
		chadd(res, dfs(pos - 1, _S, limit, flag ^ (a[pos] == 6)));
	} else {
		int up = pos == 1 ? a[1] - 1 : 9;
		for (int i = 0; i <= up; ++i) {
			int _S = (1ll * S + 1ll * i * bit[n - pos] % Mod - i + Mod) % Mod;
	//		dbg(_S);
			if (i < a[pos]) {
...
//		chadd(res, now);
	//	for (int S = 0; S < 666; ++S) {
	//		int _S = (1ll * S - 1ll * i % Mod + Mod) % Mod;
	//		int now = 0;
	//		chadd(now, dfs(n - 1, _S, 0, 1 ^ (i == 6)));
	//		if (i < a[n]) {
	//			chadd(now, dfs(n - 1, _S, 1, 1 ^ (i == 6)));
	//		}
	//		if (now) dbg(i, S, now);
	//		chadd(res, now);
	//	}
