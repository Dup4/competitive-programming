...
public:
	...
	//判断是否方阵 
	bool is_square()const { if (m == n) return 1; else return 0; } 
	//判断两个矩阵是否同型	
	bool is_same(Matrix ma) const {
		if (m == ma.m && n == ma.n)
			return 1;
		else
			return 0;
	...
	//计算逆矩阵，返回0表示非方阵1表示有结果2表示奇异矩阵 
	int inverse(Matrix &res) const {
		Matrix temp = *this;
		if (!is_square()) return 0;
		db d;
		temp.det(d);
