...
inline void prework(vector<Point> &v) {
    for (auto &it : v) {
        ...
        it.arc[0] = Frac(sqr(tmp * o), tmp.len());
        it.arc[0].x *= -1;
        tmp = Point(it.x - x[1], it.y - y[1]);
        it.arc[1] = Frac(sqr(tmp * o1), tmp.len());
        it.arc[1].x *= -1;
    }
}
...
int main() {
    ...
    for (int kase = 1; kase <= _T; ++kase) {
        ...
        prework(vec[0]);
        prework(vec[1]);
        for (int i = 0; i < 2; ++i) gao(res[i], vec[i]);
        int id = 0;
        if (small(res[1], res[0]))
            id = 1;
        print(res[id].size());
        putc('\n');
