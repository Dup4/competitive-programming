...
int n, m, q, ans[M], a[N][N]; char s[N][N];
struct E { pII a, b; };
...
bool ok(int x1, int y1, int x2, int y2) {
	...
	int midx = (x1 + x2) >> 1;
	int midy = (y1 + y2) >> 1;
	return check(x1, y1, midx, midy, 1) &&
		   check(x1, midy + 1, midx, y2, 2) &&
		   check(midx + 1, y1, x2, midy, 3) &&
...
struct BIT { 
	int a[N][N], pos[N][N];
	int T;
	void init() { memset(a, 0, sizeof a); T = 0; }
	void update(int x, int y, int v) {
		for (int i = x; i < N; i += i & -i) {
			for (int j = y; j < N; j += j & -j) {
				if (pos[i][j] == T) chmax(a[i][j], v);
				else a[i][j] = v;
			}
		}
	}
...
void run() {
	...
	for (int i = 1; i <= n; ++i) {
		for (int j = 1; j <= m; ++j) {
			...
			else if (s[i][j] == 'Y') a[i][j] = 3;
			else if (s[i][j] == 'B') a[i][j] = 4; 	
		}
	}
	rmq.init(n, m, a);
	...
	for (int i = n; i >= 1; --i) {
		...
		for (int j = m; j >= 1; --j) {
			for (auto &it : add[id(i, j)]) {
				...
				if (i > 1) add[id(i - 1, j)].push_back(it);
			}
