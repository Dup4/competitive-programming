...
inline ll calc(int dis) {
	...
	*a = unique(a + 1, a + 1 + *a) - a - 1; 
	...
	for (int i = n; i >= 1; --i) {
		...
		A.push_back({-1, p[i].fi - dis - 1, l, r});
		B.push_back({1, p[i].fi + dis, l, r});
	}
	for (int i = 1, sze = n * 2; i <= sze; ++i) {
		if (A.empty()) vec.push_back(B.back()), B.pop_back();
		else if (B.empty()) vec.push_back(A.back()), A.pop_back();
		else if (A.back().x < B.back().x) {
			vec.push_back(A.back()); A.pop_back();
		} else {
			vec.push_back(B.back()); B.pop_back();
		}
	}
	int pos = 0; 
	bit.init(m);
	for (auto &it : vec) {
		while (pos < n && p[pos + 1].fi <= it.x) {
			++pos;
