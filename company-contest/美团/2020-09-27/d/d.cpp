...
bool bfs() {
    ...
    while (!que.empty()) {
        ...
        if (remind < _a) {
            ...
            if (_t + tot - remind < res)
                que.push(node(_t + tot - remind, x, y));
            if (ex > x && _t + 1 < res)
                que.push(node(_t + 1, x + 1, y));
            else if (ex < x && _t + 1 < res)
                que.push(node(_t + 1, x - 1, y));
        } else {
            // if (ey == y && _t + _a - remind < res)
            if (_t + _a - remind < res)
                que.push(node(_t + _a - remind, x, y));
            if (ey > y && _t + 1 < res)
                que.push(node(_t + 1, x, y + 1));
            else if (ey < y && _t + 1 < res)
                que.push(node(_t + 1, x, y - 1));
        }
    }
    if (res <= limit) {
