...
struct E { int pos, l, r; };
vector <vector<E>> query;
	...
	while (scanf("%d%d%d", &n, &m1, &m2) != EOF) {
		...
		for (int i = 1, x[2], y[2]; i <= m2; ++i) {
			...
			for (int j = x[0]; j <= x[1]; ++j) {
				query[j].push_back({i, y[0], y[1]});
			}
		}	
		for (int i = 1; i <= n; ++i) if (!query[i].empty()) {
			rmq.init(n, a[i]);
			for (auto &it : query[i]) {
				ans[it.pos] = max(ans[it.pos], rmq.queryMax(it.l, it.r));
			}
		}
		for (int i = 1; i <= m2; ++i)
			printf("%lld\n", ans[i]);
