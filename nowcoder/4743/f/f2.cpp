...
struct VSEG {
	...
	int tot;
	void init() { tot = 0; } 
	int newnode() { int now = ++tot; t[now].init(); return now; } 
	void pushup(int id) {
		t[id].Max = 0;
...
struct BIT {
	int t[N]; int n;
	void init(int _n) { n = _n; memset(t, 0, sizeof t); }
	void update(int x, int p, ll v) { for (; x <= n; x += x & -x) vseg.update(t[x], 1, vLimit, p, v); } 
	ll query(int x, int p) {
		ll res = 0;
		for (; x; x -= x & -x)
			chmax(res, vseg.query(t[x], 1, vLimit, 1, p));
		return res;
	}
}bit;
...
struct BIT1 {
	int t[N], n;
	void init(int _n) { n = _n; memset(t, 0, sizeof t); }
	void update(int x, int p, ll v) { for (; x; x -= x & -x) vseg.update(t[x], 1, vLimit, p, v); }
	ll query(int x, int p) {
		ll res = 0;
		for (; x <= n; x += x & -x)
			chmax(res, vseg.query(t[x], 1, vLimit, 1, p));
