...
void dfs2(int u) {
	if (g[u].y != u && g[u].y) G.push_back({u, g[u].y, g[u].z});
	if (f[u][0].y != u && f[u][0].y) G.push_back({u, f[u][0].y, f[u][0].z});
	if (f[u][1].y != u && f[u][1].y) G.push_back({u, f[u][1].y, f[u][1].z});
	for (int i = h[u]; i; i = e[i].nx) {
		int v = e[i].v; ll w = e[i].w;
		if (v == fa[u]) continue;
		g[v] = g[u];
