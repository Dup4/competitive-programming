...
ll res;
int fa[N], in[N], out[N], r[N << 1]; ll dis[N]; int _dis[N]; 
void dfs(int u) {
	r[++*r] = u;
	in[u] = *r;
...
struct SEG {
	...
	int cnt;
	void init() { cnt = 0; t[0].val = t[0].LM = t[0].MR = INF; }
	int newnode(int val) { ++cnt; t[cnt].init(val); return cnt; }
	...
	//	return;
		...
		if (l >= ql && r <= qr) {
			...
			if (ql < l) res.MR = min({res.MR, t[id].MR, -2ll * rmq.queryMin(ql, l - 1) + t[id].val}); 
			return;
		}
		if (ql <= mid) query(t[id].ls, l, mid, ql, qr);
		if (qr > mid) query(t[id].rs, mid + 1, r, ql, qr);
	}
}seg;
...
struct TSEG {
	...
	void update(int id, int l, int r, int x, int pos, int v) {
		...
		if (l == r) return;
		int mid = (l + r) >> 1;
		if (x <= mid) update(id << 1, l, mid, x, pos, v);
		else update(id << 1 | 1, mid + 1, r, x, pos, v); 
	} 
	...
	//	dbg(id, l, r, ql, qr);
		...
		int mid = (l + r) >> 1;
		if (ql <= mid) query(id << 1, l, mid, ql, qr, pos);
		if (qr > mid) query(id << 1 | 1, mid + 1, r, ql, qr, pos);
	}
}tseg;
