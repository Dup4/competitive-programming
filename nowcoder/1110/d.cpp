...
struct DT {
	...
	void init(int _rt) {
		...
		for (int u = 1; u <= n; ++u) if (vis[u]) {
			for (auto &v : G[0][u]) if (vis[v]) ++d[v];
		}
		for (int u = 1; u <= n; ++u) if (vis[u] && d[u] == 0) { 
			que[++qr] = u;
			add(rt, u);
		}
		while (ql <= qr) {
			int u = que[ql]; ++ql;
			for (auto &v : G[0][u]) if (vis[v]) {
				if (--d[v] == 0) {
					que[++qr] = v;
					add(getlca(v), v); 
				}
