...
struct SEG { 
	struct node {
		int sum, lazy, ls, rs; 
		node() {} 
		void init() { sum = 0; lazy = -1; ls = rs = 0; }
	...
	void down(int id, int l, int r) {
		...
		if (lazy != -1) { 
			int mid = (l + r) >> 1;
			if (t[id].ls == 0) t[id].ls = newnode();
			if (t[id].rs == 0) t[id].rs = newnode();
...
void update(int l, int r, int x) {
	if (pos[l] == pos[r]) forceUpdate(l, r, x);
	else {
...
int query(int l, int r, int x) {
	if (pos[l] == pos[r]) return forceQuery(l, r, x);
	else {
