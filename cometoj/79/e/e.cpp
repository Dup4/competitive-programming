...
struct SEG {
	...
	int query(int id, int l, int r, int x) {
		...
		int mid = (l + r) >> 1;
		down(id);
		if (t[id << 1 | 1].Max >= x) return query(id << 1 | 1, mid + 1, r, x);
	...
	int query1(int id, int l, int r, int ql, int qr) {
		...
		down(id);
		int res = -INF;
		if (ql <= mid) res = max(res, query1(id << 1, l, mid, ql, qr));
		if (qr > mid) res = max(res, query1(id << 1 | 1, mid + 1, r, ql, qr));
		return res;
	}
}seg;
...
int main() {
	while (scanf("%d%d", &n, &q) != EOF) {
		op.clear(); op.resize(n + 5);
		query.clear(); query.resize(n + 5);
		seg.build(1, 1, q);
		for (int i = 1; i <= q; ++i) {
			static char sop[10];
			static int x, y;
