...
void ins(int &k, int K, int p, int l, int r) {
    ...
    int mid = (l + r) >> 1;
    if (p <= mid)
        ins(lc[k], lc[K], p, l, mid);
    else
        ins(rc[k], rc[K], p, mid + 1, r);
...
int qry(int k1, int k2, int k3, int k4, int l, int r) {
    ...
    if (l == r)
        return cnt[k2] - cnt[k1] > cnt[k4] - cnt[k3];
    int mid = (l + r) >> 1;
...
int main() {  // freopen("1.in","r",stdin);freopen("1.out","w",stdout);
    ...
    mi[0] = 1;
    rep(i, 1, N - 1) mi[i] = 1ll * mi[i - 1] * bas % mo;
    read(n);
    read(k);
    ...
    while (1) {
        ll tot = 0;
        rep(i, 1, n) if (l[i] <= r[i]) tot += r[i] - l[i] + 1;
        ...
        if (cc <= 1.5 * n) {
            ...
            for (int i = n, j = n + 1; i >= 1; i--) {
                ...
                if (l[i] <= r[i])
                    p[i] = max(min(j, r[i] + 1), l[i]), gengxiao += r[i] - p[i] + 1;
            }
        }
        ...
        if (gengxiao + 1 < k) {
            rep(i, 1, n) if (l[i] <= r[i]) r[i] = p[i] - 1;
            k -= gengxiao + 1;
            r[L] = R - 1;
        } else {
