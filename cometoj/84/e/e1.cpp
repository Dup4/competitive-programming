...
vector <W> get(int x) {
	...
	for (auto &it : vec[x]) {
		while (!pre.empty() && pre.back() < it.l - 1) {
			if (pre.back() > now.back()) now.push_back(pre.back());
			pre.pop_back();
		}
		if (it.l - 1 > now.back()) now.push_back(it.l - 1);
		if (it.l > now.back()) now.push_back(it.l); 
	}
	while (!pre.empty()) {
		if (pre.back() > now.back()) now.push_back(pre.back());
		pre.pop_back();
	...
	for (auto &it : vec[x]) {
		...
		if (it.r - 1 > now.back()) now.push_back(it.r - 1);
		if (it.r > now.back()) now.push_back(it.r);
	}
	while (!pre.empty()) {
		if (pre.back() > now.back()) now.push_back(pre.back());
		pre.pop_back();
...
pII gao(int x) {
	if (vec[x].empty()) return pII(0, 0); 
	vector <W> h = get(x);
	for (int i = 0, sze = h.size(); i < sze; ++i) {
		if (i) h[i].x = h[i].p - h[i - 1].p;
		else h[i].x = 1;
		if (i < sze - 1) h[i].y = h[i + 1].p - h[i].p;
		else h[i].y = 1;
	}
//	for (auto &it : h) dbg(it.p, it.x, it.y);
	int l = 0, r = -1; 
	int ql = 0, qr = 0;
	vector <W> g; g.push_back({0, 0, 1}); 
	pII pre = pII(0, 0), res = pII(0, 0);  
	//dbg(x); 
	for (auto &it : vec[x]) {
		for (; r < 0 || h[r].p < it.r - 1; ) { 
			++r;
			while (qr < (int)g.size() - 1 && g[qr + 1].p <= h[r].p) ++qr; 
			if (g[qr].x > pre.fi) { 
				pre = pII(g[qr].x, 1ll * g[qr].y * h[r].y % mod); 
		//	   	dbg(pre.fi, g[qr].y, h[r].y, pre.se);	
			} else if (g[qr].x == pre.fi) {
				chadd(pre.se, 1ll * g[qr].y * h[r].y % mod);
			}
		}
		for (; h[l].p < it.l - 1;) {
			while (ql < (int)g.size() - 1 && g[ql + 1].p <= h[l].p) ++ql;  
			if (g[ql].x == pre.fi) { 
				chadd(pre.se, (1ll * -g[ql].y * h[l].y % mod + mod) % mod);  
			}
			++l;
		}
		pII now = pre; ++now.fi; 
