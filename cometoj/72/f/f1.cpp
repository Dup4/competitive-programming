...
struct Block {
    ...
    void LocalShift(int l, int r) {
        ...
        for (int i = l; i < r; i++) num[GetPosition(i)] = num[GetPosition(i + 1)];
...
void IntervalShift(int l, int r) {
    ...
    for (int b_id = b_r - 1; b_id > b_l; b_id--) B[b_id].Shift(tmp, tmp);
...
void IntervalAdd(int l, int r) {
    ...
    for (int b_id = b_l + 1; b_id < b_r; b_id++) B[b_id].Add(1);
