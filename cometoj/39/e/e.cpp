...
struct BIT {
	int a[N << 1], n;
	void init(int _n) { n = _n; memset(a, 0, sizeof (a[0]) * (n + 10)); }
...
int main() {
	...
	while (_T--) {
		...
		a[0] = b[0] = 1; 
		a[n + 1] = 3; b[m + 1] = 3;
		A.init(OFFSET * 2); B.init(OFFSET * 2);
		for (int i = 1; i <= 3; ++i) {
			sa[i][0] = sb[i][0] = 0;
		...
		for (int i = Max; i >= 0; --i) {
			while (!(sa[1][posa] == i && a[posa] == 1)) { --posa; }
			while (!(sb[1][posb] == i && b[posb] == 1)) { --posb; }
			while (z < Maxz) { 	
				++z;
				while (!(sa[3][n] - sa[3][posa2 - 1] == z && a[posa2] == 3)) { --posa2; }
				while (!(sb[3][m] - sb[3][posb2 - 1] == z && b[posb2] == 3)) { --posb2; }
			//	dbg(i, z, posa2, posb2);
			...
			chmax(res, i + max(A.query(sa[2][posa] - sb[2][posb] + OFFSET) - sb[2][posb], B.query(sb[2][posb] - sa[2][posa] + OFFSET) - sa[2][posa]));
			//dbg(i, res);
		}
		printf("%d\n", res);
	}
