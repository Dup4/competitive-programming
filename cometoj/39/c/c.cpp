...
vector <int> gao(int x) {
	...
	for (int i = 1; i < n && (int)row.size() < x; ++i) {
		num += query(i, 1, i, m);
		if (num > unit) return vector<int>(K, INF);
		if (num == unit) {
			row.push_back(i);
			num = 0;
		} 
	}
	if (row.size() != x) return vector<int>(K, INF);
	int y = K - x;
	if (unit % (y + 1)) return vector<int>(K, INF);
	unit = unit / (y + 1); 
	...
	for (int i = 1; i < m && (int)col.size() < y; ++i) {
		...
		if (cnt[x] > unit) return vector<int>(K, INF);
		if (cnt[x] != unit) ok = 0;
		if (ok) {
			cnt.clear(); cnt.resize(x + 1);
			col.push_back(n - 1 + i);
		}
	}
	if ((int)(col.size() + row.size()) != K) return vector<int>(K, INF);
