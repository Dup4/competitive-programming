...
int main() {
    while (scanf("%s", s + 1) != EOF) {
        ...
        for (int i = len; i > 1; --i) {
            ...
            res += (1ll * prenum[i - 1] * sufnum % p * j % p * j % p) +
                   (1ll * sufsum * prenum[i - 1] % p - 1ll * prenum[i - 1] * sufnum % p -
                           1ll * sufnum * presum[i - 1] % p) *
                           j % p -
                   1ll * presum[i - 1] * sufsum + 1ll * presum[i - 1] * sufnum % p;
            res = (res + p) % p;
        }
        printf("%lld\n", res);
    }
