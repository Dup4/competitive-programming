...
int main() {
	while (scanf("%d", &n) != EOF) {
		...
		G.clear(); G.resize(n + 1); white.clear();
		...
		lca.bfs();
		pd.init();
		for (int i = 1; i <= n; ++i) if (col[i] == 0) white.push_back(i);
