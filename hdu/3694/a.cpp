...
struct Polygon {
    ...
    //多边形费马点
    //到所有顶点的距离和最小
    Point getFermatPoint() {
        Point now(0, 0), nx(0, 0);
        for (int i = 0; i < sze(); ++i) now = now + p[i];
        now = now / sze();
        while (1) {
            db topx = 0, topy = 0, bot = 0;
            for (int i = 0; i < sze(); ++i) {
                db d = sqrt(sqrt(fabs(now.x - p[i].x)) + sqrt(fabs(now.y - p[i].y)));
                topx += p[i].x / d;
                topy += p[i].y / d;
                bot += 1.0 / d;
            }
            nx = Point(topx / bot, topy / bot);
            if (now == nx)
                break;
            now = nx;
        }
...
Point gao1(Polygon po) {
    db t = 100000000, pd = 0.99;
    Point now(0, 0);
    ...
    while (t > eps) {
        ...
        for (int i = 0; i < po.sze(); ++i) {
            dx += (po[i].x - now.x) / (po[i].dis(now));
            dy += (po[i].y - now.y) / (po[i].dis(now));
        }
        dx *= t, dy *= t;
        db other = 0;
        for (int i = 0; i < po.sze(); ++i) other += po[i].dis(Point(now.x + dx, now.y + dy));
        if (sgn(other - res) <= 0) {
            res = other;
            now.x += dx, now.y += dy;
        }
...
db calc(Point p, Polygon &po) {
    db res = 0;
    for (int i = 0; i < po.sze(); ++i) res += po[i].dis(p);
    return res;
}
...
int main() {
    ...
    while (scanf("%lf%lf%lf%lf%lf%lf%lf%lf", x, y, x + 1, y + 1, x + 2, y + 2, x + 3, y + 3) != EOF) {
        int ok = 0;
        for (int i = 0; i < 4; ++i) {
            if (sgn(x[i] + 1) || sgn(y[i] + 1))
                ok = 1;
        }
        if (!ok)
            break;
        Polygon po;
        for (int i = 0; i < 4; ++i) po.add(Point(x[i], y[i]));
        Point p = PolygonFermatPoint::gao(po);
        db res = 0;
        for (int i = 0; i < po.sze(); ++i) res += p.dis(po[i]);
        printf("%.4f\n", res);
    }
    return 0;
}
