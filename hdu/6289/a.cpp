...
int main() {
	...
	while (_T--) {
		...
		for (int i = 2; i <= n; ++i) {
			for (int o = 0; o <= k; ++o) {
				for (int l = 0; l <= k; ++l) if (f[i - 1][1][o][l] != -1) {
					f[i][1][o + 1][l] = max(f[i][1][o + 1][l], f[i - 1][1][o][l]);
					f[i][1][o][l] = max(f[i][1][o][l], f[i - 1][1][o][l] + a[i][1]); 	
				}
			}
		}	
		for (int i = 2; i <= m; ++i) {
			for (int o = 0; o <= k; ++o) {
				for (int l = 0; l <= k; ++l) if (f[1][i - 1][o][l] != -1) {
					f[1][i][o + 1][l] = max(f[1][i][o + 1][l], f[1][i - 1][o][l]);
					f[1][i][o][l] = max(f[1][i][o][l], f[1][i - 1][o][l] + a[1][i]);
				}
			}
		}	
		for (int i = 2; i <= n; ++i) {
