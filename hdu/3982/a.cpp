...
int main() {
    ...
    for (int kase = 1; kase <= _T; ++kase) {
        ...
        scanf("%lf%d", &cir.r, &n);
        halfplanes hps;
        Point a(-cir.r - 5, -cir.r - 5);
        Point b(-cir.r - 5, cir.r + 5);
        Point c(cir.r + 5, cir.r + 5);
        Point d(cir.r + 5, -cir.r - 5);
        hps.add(halfplane(a, b));
        hps.add(halfplane(b, c));
        hps.add(halfplane(c, d));
        hps.add(halfplane(d, a));
        for (int i = 1; i <= n; ++i) {
            Line li;
            li.scan();
            hps.add(li);
        ...
        Point center;
        center.scan();
        for (int i = 0; i < hps.sze(); ++i) {
            if (hps[i].relationPoint(center) == 2)
                swap(hps[i].s, hps[i].e);
        }
        hps.halfPlaneIntersect();
        Polygon po;
        po = hps.getConvex();
        printf("%.5f%%\n", po.areaIntersectCircle(cir) / cir.area() * 100);
    }
    return 0;
}
