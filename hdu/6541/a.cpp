...
struct Polygon {
    ...
    void getUpDownHull(Polygon &upHull, Polygon &downHull) {
        ...
        for (int i = 0; i < sze(); ++i) {
            ...
            upHull.add(p[i]);
            while (downHull.sze() > 1 && sgn((p[i] - downHull[-1]) ^ (downHull[-2] - downHull[-1])) <= 0)
                downHull.p.pop_back();
            downHull.add(p[i]);
        }
    }
...
Polygon minKowSki(Polygon &A, Polygon &B) {
    ...
    while (i < A.sze() && j < B.sze()) {
        if (sgn((A[i + 1] - A[i]) ^ (B[j + 1] - B[j])) >= 0) {
            res.add(res[-1] + (A[i + 1] - A[i]));
            ++i;
        } else {
            res.add(res[-1] + (B[j + 1] - B[j]));
            ++j;
        }
    }
    while (i < A.sze()) {
...
int main() {
    ...
    while (scanf("%d%d", &n, &q) != EOF) {
        ...
        for (int i = 1; i <= q; ++i) {
            ...
            db ans = 0;
            if (p.y > 0)
                ans = max(ans, upHull.querySlopeMax(p));
            else
                ans = max(ans, downHull.querySlopeMax(p));
            printf("%lld\n", ans);
        }
    }
    return 0;
