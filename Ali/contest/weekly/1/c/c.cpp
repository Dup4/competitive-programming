...
public:
    ...
    vector<int> ProductList(int offset, int n, int len1, int len2) {
		...
		vector <int> res;
		res.push_back(min(len1, offset));
		res.push_back(min(len1, offset + n));
		res.push_back(min(max(offset - len1, 0), len2));
		res.push_back(min(max(offset + n - len1, 0), len2));
		return res;
    }
};

void run() {
	rd(offset, n, len1, len2);
	pt((new Solution())->ProductList(offset, n, len1, len2));
}
