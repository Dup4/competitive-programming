...
struct Point2 {
	...
	bool operator < (const Point2 &b) const { return sgn(x - b.x) == 0 ? sgn(y - b.y) < 0 : x < b.x; }
	Point2 operator + (const Point2 &b) const { return Point2(x + b.x, y + b.y); }
	Point2 operator - (const Point2 &b) const { return Point2(x - b.x, y - b.y); }
	Point2 operator * (const db &b) const { return Point2(x * b, y * b); }
	Point2 operator / (const db &b) const { return Point2(x / b, y / b); }
	db operator ^ (const Point2 &b) const { return x * b.y - y * b.x; }
	db operator * (const Point2 &b) const { return x * b.x + y * b.y; }
	db len() { return hypot(x, y); }
	db dis(Point2 b) { return hypot(x - b.x, y - b.y); }
};
...
struct Line2 {
	...
	//点到直线的距离
	db disPoint2ToLine2(Point2 p) { return fabs((p - s) ^ (e - s)) / length(); } 
	//点到线段的距离
...
public:
	...
	//	dbg(l.disPoint2ToSeg(B), ra + rb);
		if (sgn(l.disPoint2ToSeg(B) - ra - rb) <= 0) return 1;
		return -1;
    }
};
