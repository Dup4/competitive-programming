...
struct Dijkstra {
    ...
    long long getSecondDiameter(vector<vector<int>> &edge) {
		...
		G.init(n + 5);
		for (auto &it : edge) {
			G.addedge(it[0] + 1, it[1] + 1, it[2]);
			G.addedge(it[1] + 1, it[0] + 1, it[2]);
		}
		dij.gao(1, G);
		vector <pLI> vec;
		for (int i = 1; i <= n; ++i) {
			vec.push_back(pLI(dij.dis[i], i));
		}
		sort(all(vec), [&](pLI x, pLI y) { return x.fi > y.fi; });
		map <pII, int> mp;
		mp.clear();
		ll Max = 0, MMax = 0;
		for (int i = 0; i < min(10, n); ++i) {
			...
			for (int j = 1; j <= n; ++j) {
				int _v = j; 
				pII it = pII(min(_u, _v), max(_u, _v));
				if (mp[it] == 1) continue;
				mp[it] = 1;
				ll dis = dij.dis[j];
				if (dis >= Max) {
