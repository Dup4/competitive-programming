...
namespace MoTree {
	...
	struct mnode { int x, y, b; }upd[N]; int upds;
	void addQuery(int u, int v) { ++ques; que[ques] = { u, v, upds, ques}; }
	void addModify(int x, int y) { ++upds; upd[upds] = { x, y, 0}; }
	ll ans[N], now;
	int b[N], vis[N];
	void back(int t) {
	    if (vis[upd[t].x]) {
	...
	void gao() {
		...
		*belong = 0; *id = 0; fa[1] = 1;
	    ...
	    for (int i = 1; i <= n; i++) {
	        if (id[que[i].u] > id[que[i].v]) swap(que[i].u, que[i].v);
		}
    	sort(que + 1, que + 1 + ques);
	    int u = 1, v = 1, t = 0;
	    for (int i = 1; i <= ques; i++) {
	        if (u != que[i].u) move(u, que[i].u), u = que[i].u;
	        if (v != que[i].v) move(v, que[i].v), v = que[i].v;
	        int f = querylca(u, v);
	        update(f);
...
int main() {
	...
	for (int i = 1; i <= q; ++i) {
		int op, x, y;
		scanf("%d%d%d", &op, &x, &y);
		if (!op) MoTree::addModify(x, y);
		else MoTree::addQuery(x, y);
	}
	MoTree::gao();
    return 0;
}
