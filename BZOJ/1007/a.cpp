...
struct E {
    ...
    bool operator<(const E &other) const {
        if (sgn(A - other.A) != 0)
            return sgn(A - other.A) > 0;
        return sgn(B - other.B) < 0;
    }
} sta[N], li[N];
int top;
void insert(E e) {
    while (top) {
        if (sgn(e.A - sta[top].A) == 0)
            --top;
        else if (top > 1 && e.li.relationPoint(sta[top].li.crossPoint(sta[top - 1].li)) >= 2)
            --top;
        else
            break;
    }
...
int main() {
    ...
    for (int i = 1; i <= n; ++i) {
        db A, B;
        scanf("%lf%lf", &A, &B);
        li[i] = {A, B, i, Line(A, -1, B)};
    }
    top = 0;
    sort(li + 1, li + 1 + n);
    for (int i = 1; i <= n; ++i) insert(li[i]);
    vector<int> res;
    for (int i = 1; i <= top; ++i) res.push_back(sta[i].id);
