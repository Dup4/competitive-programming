...
struct SEG {
    ...
    void ins(int id, int l, int r, int pos, Point p) {
        ...
        int mid = (l + r) >> 1;
        if (pos <= mid)
            ins(id << 1, l, mid, pos, p);
        else
            ins(id << 1 | 1, mid + 1, r, pos, p);
    }
    db query(int id, int l, int r, int ql, int qr, Point p) {
        if (l >= ql && r <= qr) {
            if (p.y > 0)
                return upHull[id].querySlopeMax(p);
            else
                return downHull[id].querySlopeMax(p);
        }
        int mid = (l + r) >> 1;
        db Max = -1e18;
        if (ql <= mid)
            Max = max(Max, query(id << 1, l, mid, ql, qr, p));
        if (qr > mid)
            Max = max(Max, query(id << 1 | 1, mid + 1, r, ql, qr, p));
        return Max;
    }
} seg;
