...
struct SAM {
	...
	struct node {
		...
		node() { maxlen = fa = rt = 0; memset(nx, 0, sizeof nx); }
	}t[N << 1];
	int tot, lst;
	vector < vector<int> > G;
	inline int newnode() { 
	...
	void build(char *s) { 
		...
		for (int i = 1; i <= tot; ++i) if (t[i].fa) G[t[i].fa].push_back(i);
		DFS(1);
	}
	inline bool check(int u, int len, int l, int r, int c) {
		if (l + len > r) return 0;
		if (!t[u].nx[c]) return 0;
		u = t[u].nx[c];
...
struct SAMT {
	...
	struct node {
		...
		node() { maxlen = plen = fa = 0; memset(nx, 0, sizeof nx); }
	}t[N << 2];
	int tot, lst;
	inline int newnode() { 
		++tot;
		...
		for (int _q = 1; _q <= q; ++_q) {
			...
			for (int i = 0, u = 1, len = 0; t[i]; ++i) {
				...
				while (u && !sam.check(u, len, l, r, c)) {
					if (--len <= sam.t[sam.t[u].fa].maxlen) u = sam.t[u].fa;
				}
				if (!u) u = 1, len = 0;
				else u = sam.t[u].nx[c], ++len;
				samt.extend(c, len);
			}
			samt.calc();
		}
