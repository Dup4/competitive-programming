...
struct SEG {
	...
	void update(int id, int l, int r, int ql, int qr, int c) {
		...
		int mid = (l + r) >> 1;
		pushdown(id);
		if (ql <= mid) update(id << 1, l, mid, ql, qr, c);
		if (qr > mid) update(id << 1 | 1, mid + 1, r, ql, qr, c);
		t[id] = t[id << 1] + t[id << 1 | 1];
	}
	void query(int id, int l, int r, int ql, int qr, node &res) {
		...
		int mid = (l + r) >> 1;
		pushdown(id);
		if (qr > mid) query(id << 1 | 1, mid + 1, r, ql, qr, res);
		if (ql <= mid) query(id << 1, l, mid, ql, qr, res);
	}
}seg;
