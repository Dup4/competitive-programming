...
struct Graph {
	struct E {int to, nx;}e[N << 2];
	int h[N];
	void init() { memset(h, 0, sizeof (h[0]) * (n + 10)); }
	void addedge(int u, int v) { e[++*h] = { v, h[u] }; h[u] = *h; }
}G[2];
...
struct Tarjan {
	...
	//ptcc[i] 表示第i个点分中的点 
	//cut[i]　表示点i是否是割点
	int Low[N], DFN[N], sta[N], num, tot, Belong[N]; bool cut[N]; 
    vector <int> ptcc[N]; 
