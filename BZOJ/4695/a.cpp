...
struct SEG {
	...
	void update1(int id, int l, int r, int ql, int qr, int x) {
		...
		int mid = (l + r) >> 1;
		pushdown(id);
		if (ql <= mid) update1(id << 1, l, mid, ql, qr, x);
		if (qr > mid) update1(id << 1 | 1, mid + 1, r, ql, qr, x);
		t[id] = t[id << 1] + t[id << 1 | 1];
	}
	void update2(int id, int l, int r, int ql, int qr, int x) {
		if (x >= t[id].Max[0]) return;
		if (l == r) {
		...
		int mid = (l + r) >> 1;
		pushdown(id);
		if (ql <= mid) update2(id << 1, l, mid, ql, qr, x);
		if (qr > mid) update2(id << 1 | 1, mid + 1, r, ql, qr, x);
		t[id] = t[id << 1] + t[id << 1 | 1];
	}
