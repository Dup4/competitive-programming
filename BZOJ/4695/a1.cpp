...
struct Mininfo { int Min, Nin, Cnt, Delta; };
struct Maxinfo { int Max, Nax, Cnt, Delta; };
Mininfo operator + (Mininfo a, Mininfo b) {
	Mininfo ans;
	ans.Cnt = ans.Delta = 0;
	ans.Min = min(a.Min, b.Min);
	ans.Nin = min(a.Nin, b.Nin);
	if (a.Min != ans.Min) ans.Nin = min(ans.Nin, a.Min);
	else ans.Cnt += a.Cnt;
	if (b.Min != ans.Min) ans.Nin = min(ans.Nin, b.Min);
	else ans.Cnt += b.Cnt;
	return ans;
}
Maxinfo operator + (Maxinfo a, Maxinfo b) {
	...
	if (a.Max != ans.Max) ans.Nax = max(ans.Nax, a.Max);
	else ans.Cnt += a.Cnt;
	if (b.Max != ans.Max) ans.Nax = max(ans.Nax, b.Max);
	else ans.Cnt += b.Cnt;
	return ans;
}
...
struct SEG {
	...
	void pushdown(int root) {
		...
		if (a[root].Max.Delta) {
			...
			if (a[lc].Max.Max == a[rc].Max.Max) {
				addmax(lc, tmp);
				addmax(rc, tmp);
			} else if (a[lc].Max.Max > a[rc].Max.Max) addmax(lc, tmp);
			else addmax(rc, tmp);
			a[root].Max.Delta = 0;
		}
		if (a[root].Min.Delta) {
			...
			if (a[lc].Min.Min == a[rc].Min.Min) {
				addmin(lc, tmp);
				addmin(rc, tmp);
			} else if (a[lc].Min.Min < a[rc].Min.Min) addmin(lc, tmp);
			else addmin(rc, tmp);
			a[root].Min.Delta = 0;
		}
		if (a[root].tag) {
	...
	void add(int root, int l, int r, int ql, int qr, int val) {
		...
		pushdown(root);
		int mid = (l + r) / 2;
		if (mid >= ql) add(a[root].lc, l, mid, ql, min(qr, mid), val);
	...
	void chkmax(int root, int l, int r, int ql, int qr, int val) {
		...
		pushdown(root);
		int mid = (l + r) / 2;
		if (mid >= ql) chkmax(a[root].lc, l, mid, ql, min(qr, mid), val);
		if (mid + 1 <= qr) chkmax(a[root].rc, mid + 1, r, max(mid + 1, ql), qr, val);
		update(root);
	}
	...
	void chkmin(int root, int l, int r, int ql, int qr, int val) {
		...
		pushdown(root);
		int mid = (l + r) / 2;
		if (mid >= ql) chkmin(a[root].lc, l, mid, ql, min(qr, mid), val);
		if (mid + 1 <= qr) chkmin(a[root].rc, mid + 1, r, max(mid + 1, ql), qr, val);
		update(root);
	}
	...
	ll sum(int root, int l, int r, int ql, int qr) {
		...
		int mid = (l + r) / 2;
		ll ans = 0;
		if (mid >= ql) ans += sum(a[root].lc, l, mid, ql, min(qr, mid));
		if (mid + 1 <= qr) ans += sum(a[root].rc, mid + 1, r, max(mid + 1, ql), qr);
		return ans;
	}
	ll sum(int l, int r) {
	...
	ll Max(int root, int l, int r, int ql, int qr) {
		if (l == ql && r == qr) return a[root].Max.Max;
		pushdown(root);
		int mid = (l + r) / 2;
		ll ans = -INF;
		if (mid >= ql) ans = max(ans, Max(a[root].lc, l, mid, ql, min(qr, mid)));
		if (mid + 1 <= qr) ans = max(ans, Max(a[root].rc, mid + 1, r, max(mid + 1, ql), qr));
		return ans;
	}
	ll Max(int l, int r) {
	...
	ll Min(int root, int l, int r, int ql, int qr) {
		if (l == ql && r == qr) return a[root].Min.Min;
		pushdown(root);
		int mid = (l + r) / 2;
		ll ans = INF;
		if (mid >= ql) ans = min(ans, Min(a[root].lc, l, mid, ql, min(qr, mid)));
		if (mid + 1 <= qr) ans = min(ans, Min(a[root].rc, mid + 1, r, max(mid + 1, ql), qr));
		return ans;
	}
	ll Min(int l, int r) { return Min(root, 1, n, l, r); }
}seg;
...
int main() {
	...
	for (int i = 1; i <= m; i++) {
		...
		if (opt <= 3) read(val);
		if (opt == 1) seg.add(l, r, val);
		if (opt == 2) seg.chkmax(l, r, val);
		if (opt == 3) seg.chkmin(l, r, val);
		if (opt == 4) writeln(seg.sum(l, r));
		if (opt == 5) writeln(seg.Max(l, r));
		if (opt == 6) writeln(seg.Min(l, r));
	}
	return 0;
}
