...
struct LCA {
	...
	inline int dis(int u, int v) { return deep[u] + deep[v] - 2 * deep[querylca(u, v)]; }
	inline void init() { fa[1] = 1; deep[1] = 0; DFS(1); gettop(1, 1); }
}lca;
...
struct SEG {
	...
	void update(int &now, int l, int r, int pos, int v) {
		if (!now) now = ++tot, t[now].sum = t[now].ls = t[now].rs = 0;
	    t[now].sum += v;	
		if (l == r) return;
		int mid = (l + r) >> 1;
...
namespace PD {
	...
	int query(int x,int K) {
	    ...
	    for(int i = x; fa[i]; i = fa[i]) {
	        int dis = lca.dis(x, fa[i]); if(dis > K) continue;
