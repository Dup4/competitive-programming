...
inline int get(int x) { return lower_bound(b + 1, b + 1 + *b, x) - b; }
...
struct SEG {
	...
	int query(int tl, int tr, int l, int r, int ql, int qr) { 
		if (ql > qr || tr == 0) return 0;
		if (l >= ql && r <= qr) {
			return t[tr].sum - t[tl].sum;
		}
		int mid = (l + r) >> 1;
		int res = 0;
		if (ql <= mid) res += query(t[tl].ls, t[tr].ls, l, mid, ql, qr);
		if (qr > mid) res += query(t[tl].rs, t[tr].rs, mid + 1, r, ql, qr);
		return res;
	}
}seg;
