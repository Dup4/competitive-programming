...
int main() {
	scanf("%d", &n); int rt = n + 1;
	for (int i = 1; i <= n; ++i) lct.t[i] = LCT::node(), lct.t[i].sum = 1;
	for (int i = 1; i <= n; ++i) {
		scanf("%d", &lct.t[i].v);
		lct.link(i, min(rt, i + lct.t[i].v)); 
	}	
	int op, x, y;
	scanf("%d", &q);
	while (q--) {
		...
		if (op == 1) {
			...
			scanf("%d", &y);
			lct.cut(min(rt, x + lct.t[x].v), x);
			lct.t[x].v = y;
			lct.link(min(rt, x + lct.t[x].v), x);
		}
	}
	return 0;
}
