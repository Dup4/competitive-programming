...
int getfather(int x) {
    ...
    if (fa[x] != y)
        stk1[++top] = x, stk2[top] = fa[x];  //如果路径压缩后x的fa会改变，就将其压入栈中0
    return fa[x] = y;                        //路径压缩
}
...
void CDQ(int lf, int rg) {
    ...
    for (int i = lf; i <= mid; i++)
        for (int j = 1; j <= q[i].cnt; j++) ed[q[i].c[j]].tim = tim;  //把所有左半区域的询问里不存在的边打上标记
    for (int i = mid + 1; i <= rg; i++)
        for (int j = 1; j <= q[i].cnt; j++) {
            ...
            if (ed[v].tim != tim) {
                ...
                if (x != y)
                    stk1[++top] = x, stk2[top] = fa[x], fa[x] = y;
            }
    ...
    tim++;
    for (int i = mid + 1; i <= rg; i++)
        for (int j = 1; j <= q[i].cnt; j++) ed[q[i].c[j]].tim = tim;  //对所有右区间不存在的边打上标记
    for (int i = lf; i <= mid; i++)
        for (int j = 1; j <= q[i].cnt; j++) {
            int v = q[i].c[j];
            if (ed[v].tim != tim) {
...
int main() {
    ...
    scanf("%d", &T);
    tim = 1;  //一开始的标记tim不能是0，这是由于所有边的标记都默认为0，无法区分
    for (int i = 1; i <= T; i++) {
        scanf("%d", &q[i].cnt);
        for (int j = 1; j <= q[i].cnt; j++) scanf("%d", &q[i].c[j]), ed[q[i].c[j]].tim = tim;
    }
