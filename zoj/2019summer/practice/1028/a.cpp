...
int n, m, q, s;
char G[N][N];
pll a[30][N][N], b[30][N][N], base[N][N], num[N][N]; 
...
int main() {
	while (scanf("%d%d%d%d", &n, &m, &q, &s) != EOF) {
		...
		for (int i = 1; i <= n; ++i) {
			for (int j = 1; j <= m; ++j) {
				addpre(base, i, j);
				add(b[G[i][j] - 'a' + 1], G[i][j] - 'a' + 1, i, j, i, j, q - num[i][j].fi);
			}
		}
		for (int i = 1; i <= 26; ++i) {
			for (int j = 1; j <= n; ++j) {
			...
			for (int j = 1; j <= n; ++j) {
				for (int k = 1; k <= m; ++k) {
					...
					b[i][j][k].fi += b[i - 1][j][k].fi;
					b[i][j][k].se += b[i - 1][j][k].se;
				}
			}
		}
		ll tot = 0;
		...
		for (int i = 1; i <= q; ++i) {
			...
			r = query(b[id + 1], x[0], y[0], x[1], y[1]);
			now += 1ll * id * l.fi - l.se - 1ll * id * r.fi + r.se;
			if (now < Min) {
				Min = now;
				pos = i;
