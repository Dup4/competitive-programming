...
void upd(int k, int v) {
    for (int i = k; i <= n; i += lowbit(i)) update(rt[i], 1, M, a[k], v);
}
int lq[maxm], rq[maxm], ltsz, rtsz;
void init(int l, int r) {
    --l;
    ltsz = rtsz = 0;
    for (; l; l -= lowbit(l)) lq[ltsz++] = rt[l];
    for (; r; r -= lowbit(r)) rq[rtsz++] = rt[r];
    // assert(ltsz <= 30 && rtsz <= 30);
}

int sum() {
    int res = 0;
    for (int i = 0; i < rtsz; ++i) res += node[node[rq[i]].ls].val;
    for (int i = 0; i < ltsz; ++i) res -= node[node[lq[i]].ls].val;
    return res;
}
...
int query(int l, int r, int k) {
    ...
    int cur = sum();
    if (cur >= k) {
        for (int i = 0; i < rtsz; ++i) rq[i] = node[rq[i]].ls;
        for (int i = 0; i < ltsz; ++i) lq[i] = node[lq[i]].ls;
        return query(l, mid, k);
    } else {
        for (int i = 0; i < rtsz; ++i) rq[i] = node[rq[i]].rs;
        for (int i = 0; i < ltsz; ++i) lq[i] = node[lq[i]].rs;
