...
int n, m, q, e[N][3], d[N], W[N], big[N], bigp[N], cntp[N], cnte[N];  
ll f[N][2], g[2];  //0 + 1 -
vector <vector<pII>> G;  
		...
		*bigp = 0; g[0] = g[1] = 0;
	    ...
	    while (q--) {
			...
			if (op[0] == 'Q') {
				...
				scanf("%d", &u);
				if (big[u]) swap(f[u][0], f[u][1]), cntp[u] ^= 1; 
				for (auto &it: G[u]) {
					int v = it.fi, &w = W[it.se];
					if (big[u] != big[v]) {
						cnte[it.se] ^= 1;
						if (cntp[u] ^ cnte[it.se]) {
							if (w < 0) {
								f[v][1] -= abs(w);
								f[v][0] += abs(w);
							} else {
