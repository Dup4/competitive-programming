...
//    static db floor_div(db a, db b) { return a / b - ((a ^ b) < 0 && a % b != 0); }
    ...
    db query(db x) {
       ...
       // set<segment>::iterator it = segments.upper_bound(segment(point(x, INF)));
        set<segment>::iterator it = segments.upper_bound(segment(point(x, 1), point(0, 0), 1)); 
...
struct SEG {
	...
	void update(int id, int l, int r, int ql, int qr, db k, db b) {
		...
		int mid = (l + r) >> 1;
		if (ql <= mid) update(id << 1, l, mid, ql, qr, k, b);
		if (qr > mid) update(id << 1 | 1, mid + 1, r, ql, qr, k, b);
	}
