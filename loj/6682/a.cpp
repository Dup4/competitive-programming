...
ll S(ll x, int y) {
	...
	int k = id(x);
	ll res = one * 4 * (g[k] - y + 1); 
	for (int i = y; i <= tot && one * pri[i] * pri[i] <= x; ++i) {
		ll t1 = pri[i], t2 = one * pri[i] * pri[i];
		for (int e = 1; t2 <= x; ++e, t1 = t2, t2 *= pri[i]) {
			res += ((sqr(e + 1)) * S(x / t1, i + 1) % mod + sqr(e + 2)) % mod;
		}
	}
	return res;
}
