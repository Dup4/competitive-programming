...
int n, q, ans[N];
char s[N];
struct qnode{int l, id;}; 
struct pnode{int u, len;};
vector <vector<pnode>> add;
vector <vector<qnode>> qvec; 
struct BIT {
	int a[N];
	void init() { memset(a, 0, sizeof a); }
	void update(int x, int v) { for (; x > 0; x -= x & -x) a[x] = max(a[x], v); }
	int query(int x) { int res = 0; for (; x < N; x += x & -x) res = max(res, a[x]); return res;}
}bit;
struct SAM {
	//空间开两倍，节点个数是两倍字符串长度
	...
	struct node {
		...
		set <int> se;
		node() { maxlen = fa = 0; memset(nx, 0, sizeof nx); se.clear(); }
	}t[N << 1];
	int tot, lst;
	...
	void build(char *s) { 
		...
		memset(h, 0, sizeof h);
		for (int i = 2; i <= tot; ++i) if (t[i].fa) addedge(t[i].fa, i);
		DFS(1);
	}
}sam;
