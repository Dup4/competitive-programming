...
struct SEG {
    ...
    void update(int &now, int pre, int l, int r, int pos, int fa, int sze) {
        ...
        int mid = (l + r) >> 1;
        if (pos <= mid)
            update(t[tmp].ls, t[pre].ls, l, mid, pos, fa, sze);
        else
            update(t[tmp].rs, t[pre].rs, mid + 1, r, pos, fa, sze);
        now = tmp;
    }
...
void join(int k, int x, int y) {
    ...
    if (fx.fi != fy.fi) {
        ...
        seg.update(seg.rt[k], seg.rt[k], 1, tot, fx.fi, fy.fi, 0);
        seg.update(seg.rt[k], seg.rt[k], 1, tot, fy.fi, fy.fi, fx.se);
    }
}
