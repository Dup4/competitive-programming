...
namespace WQS {
	...
	db slope(int k, int j, int id) { return (y[id][j] - y[id][k]) / (x[id][j] - x[id][k]); }
	pLI check(ll k) {
		head[0] = 1; tail[0] = 0;
		head[1] = 1; tail[1] = 0; 
		que[0][++tail[0]] = 0;
		for (int i = 1, j; i <= n; ++i) {
			//第i个位置是集结点的最值
			while (head[0] < tail[0] && slope(que[0][head[0]], que[0][head[0] + 1], 0) < a[i]) ++head[0];
			j = que[0][head[0]];
			...
			que[1][++tail[1]] = i;
			//前i个位置的最值计算
			while (head[1] < tail[1] && slope(que[1][head[1]], que[1][head[1] + 1], 1) < i) ++head[1];
			j = que[1][head[1]];
