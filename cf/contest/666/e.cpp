...
struct SEG {
	...
	void query(int id, int l, int r, int ql, int qr) { 
		...
		int mid = (l + r) >> 1;
		if (t[id].ls && ql <= mid) query(t[id].ls, l, mid, ql, qr);
		if (t[id].rs && qr > mid) query(t[id].rs, mid + 1, r, ql, qr);
	}
}seg;
...
struct SAM {
	...
	void DFS(int u) {
		...
		for (auto &it : qvec2[u]) {
			...
			if (seg.res.v) ans[id] = pII(seg.res.id, seg.res.v);
		}
	}
	inline void gao() {
		G.clear(); G.resize(tot + 10);
		qvec2.clear(); qvec2.resize(tot + 10);
		for (int i = 1; i <= tot; ++i) if (t[i].fa) {
