...
const int N = 2e5 + 10;
const ll mod = 1e9 + 7;  
int n, ans, a[N], b[N], pri[N], check[N], mu[N], phi[N], fa[N], deep[N], son[N], sze[N], top[N], in[N], out[N], sta[N], p[N << 1], vis[N], f[N], g[N], F[N];   
vector <vector<int>> G;
		...
		for (int T = 1; T <= n; ++T) {
			...
			*p = unique(p + 1, p + 1 + *p) - p - 1;
			for (int i = 1; i <= *p; ++i) {
				...
				if (*sta) G[sta[*sta]].push_back(p[i]);
				sta[++*sta] = p[i];
			}
			dp(p[1], p[1]); add(ans, ans); add(ans, 2ll * s1 * s2 % mod);
			for (int i = 1; i <= *p; ++i) vis[p[i]] = 0, G[p[i]].clear();
			F[T] = ans; ans = 0;
		}
		for (int i = 1; i <= n; ++i)
			for (int j = i; j <= n; j += i) if (mu[j / i]) 
				add(g[i], (1ll * mu[j / i] * F[j] % mod + mod) % mod);
		for (int i = 1; i <= n; ++i)
			add(ans, 1ll * i * qpow(phi[i], mod - 2) % mod * g[i] % mod);
