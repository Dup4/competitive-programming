...
void run() {
	...
	vector <int> A, B, C; 
	for (int i = 1; i <= n; ++i) {
		if (i == 1 || a[i].fi != a[i - 1].fi) A.push_back(i);
	}
	A.push_back(n + 1);
	sort(a + 1, a + 1 + n, [](pii x, pii y) {
		return x.se < y.se;		
