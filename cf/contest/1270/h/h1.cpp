...
struct SEG {
	...
	int queryl(int id, int l, int r, int v) {
		if (l == r) return l;	
		int mid = (l + r) >> 1;
		if (t[id << 1].Min < v) return queryl(id << 1, l, mid, v);
		else return query(id << 1 | 1, mid + 1, r, v); 
	}
	int queryr(int id, int l, int r, int v) {
		if (l == r) return l;
		int mid = (l + r) >> 1;
		if (t[id << 1 | 1].Max > v) return queryr(id << 1 | 1, mid + 1, r, v);
		else return queryr(id << 1, l, mid, v);
	}
	int calc(int id, int l, int r) {
		if (l > r) return 0;
		if (l == r) return 1;
		if (t[id << 1].Min > t[id << 1 | 1].Max) return t[id << 1].cnt + t[id << 1 | 1].cnt;
		int mid = (l + r) >> 1;
