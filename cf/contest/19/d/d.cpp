...
struct SEG {
	...
	void update(int id, int l, int r, int x, int y, int opt) {
		if (l == r) {
			if (opt == 0) t[id].s.insert(y);
			else t[id].s.erase(y);
			t[id].Max = 0;
			if (!t[id].s.empty()) chmax(t[id].Max, *t[id].s.rbegin());
			return;
		}
		int mid = (l + r) >> 1;
	...
	void query(int id, int l, int r, int qx, int qy) {
		if  (res.x != INF || qx >= r || t[id].Max <= qy) return; 
		if (l == r) { 
...
void run() {
	...
	for (int i = 1; i <= n; ++i) {
		if (e[i].op <= 1) {
			...
			else pt(hx[seg.res.x], hy[seg.res.y]);
		}
	}
}
