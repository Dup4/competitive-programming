...
struct SEG {
    ...
    void Set(int &now, int l, int r, int pos, int In, int Out, int num) {
        ...
        down(now);
        if (pos <= mid)
            Set(t[now].ls, l, mid, pos, In, Out, num);
        else
            Set(t[now].rs, mid + 1, r, pos, In, Out, num);
        up(now);
    }
    void update(int &now, int l, int r, int ql, int qr, int v) {
        ...
        down(now);
        if (ql <= mid)
            update(t[now].ls, l, mid, ql, qr, v);
        if (qr > mid)
            update(t[now].rs, mid + 1, r, ql, qr, v);
        up(now);
    }
    ll query(int &now, int l, int r, int ql, int qr) {
        ...
        ll res = 0;
        if (ql <= mid)
            res += query(t[now].ls, l, mid, ql, qr);
        if (qr > mid)
            res += query(t[now].rs, mid + 1, r, ql, qr);
        return res;
    }
} seg;
void run() {
    ...
    for (int _q = 1; _q <= q; ++_q) {
        ...
        if (d[u] <= S) {
            ...
            if (_q == 2)
                cout << "# " << seg.query(seg.rt[u], 1, n + m, w[u] + 1, n + m) << endl;
