...
void run() {
	...
	if (pos[s - Max] == -1) return pt(-1);
	vector <int> vec; vec.push_back(posMax); used[posMax] = 1;
	int it = s - Max;
	while (it) {
		used[pos[it]] = 1;
	...
	sort(other.begin(), other.end(), [&](int x, int y) { return a[x] < a[y]; });
